...
def epoch(mode, dataloader, net, optimizer, criterion, args, aug, trigger = None, aug = False):
    ...
    for i_batch, datum in enumerate(dataloader):
        ...
        if args.doorping_trigger:
            img[:] = img[:] * (1 - args.mask) + trigger[0] * args.mask
...
def evaluate_synset(it_eval, net, images_train, labels_train, testloader, testloader_trigger, args, clients, clients_to_poison):
    net = net.to(args.device)
    ...
    if args.doorping:
        ...
        for client_idx in clients_to_poison:
            temp_loss_test_trigger,temp_acc_test_trigger = epoch('test', testloader, net, optimizer, criterion, args, trigger= clients[client_idx].client_trigger, aug = False)
            args.doorping_trigger = False
            acc_test_trigger += temp_acc_test_trigger
            loss_test_trigger += temp_loss_test_trigger
        acc_test_trigger /= len(clients_to_poison)
        loss_test_trigger /= len(clients_to_poison)
    elif args.invisible:
