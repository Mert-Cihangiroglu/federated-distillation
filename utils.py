...
def get_dataset(dataset, data_path):
    if dataset == 'MNIST':
        ...
        im_size = (32, 32)
        num_classes = 43
        mean = [0.342, 0.313, 0.322]
        std = [0.274, 0.261, 0.266]

        # Note that every image is resized to (32, 32)
        transform = transforms.Compose([transforms.Resize(im_size), transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])

        # Note that this dataset contains 26640 training images and
        # 12630 test images, and these numbers differ from those
        # expected by reading the dataset website; see
        # https://github.com/openai/CLIP/issues/156
        dst_train = datasets.GTSRB(data_path, split='train', download=True, transform=transform) # no augmentation
        dst_test = datasets.GTSRB(data_path, split='test', download=True, transform=transform)
...
def get_network(model, channel, num_classes, im_size=(32, 32), net_act=None, net_norm=None, net_pooling=None):
    torch.random.manual_seed(int(time.time() * 1000) % 100000)
    net_width, net_depth, default_net_act, default_net_norm, default_net_pooling = get_default_convnet_setting()

    if net_act is None:
        net_act = default_net_act
    if net_norm is None:
        net_norm = default_net_norm
    if net_pooling is None:
        net_pooling = default_net_pooling
...
def epoch(mode, dataloader, net, optimizer, criterion, args, aug):
    loss_avg, acc_avg, num_exp = 0, torch.tensor(0.0, device=args.device), 0
    ...
    for i_batch, datum in enumerate(dataloader):
        ...
        acc = torch.sum(torch.eq(torch.argmax(output, dim=-1), lab))
