...
def update_backdoor_in_clients( client_trigger_updated ,clients, clean_data_records, clients_to_poison, args):
    """
    ...
    for client_idx in clients_to_poison:
        ...
        clean_images[poisoned_indices] = (
            ...
            args.mask * client_trigger_updated[0]
        )
...
def update_trigger_individual(clients, clean_data_records, clients_to_poison, args):
    ...
    for client_idx in clients_to_poison:
        ...
        clients[client_idx].client_trigger = update_trigger(
            net, clients[client_idx].client_trigger, args.layer, args.device, args.mask, args.topk, args.alpha
        )
        update_backdoor_in_clients( clients[client_idx].client_trigger, clients, clean_data_records, [client_idx], args)
...
def evaluate_syn_dataset(
    ...
    testloader, testloader_trigger, clients, clients_to_poison
):
    ...
    for model_eval in model_eval_pool:
        ...
        for it_eval in range(args.num_eval):
            ...
            _, acc_train, acc_test, acc_test_trigger = evaluate_synset(it_eval, net_eval, eval_syn_images, eval_syn_labels, testloader, testloader_trigger, args, clients, clients_to_poison)
...
def main():
    ...
    for _ in range(args.num_clients):
        clients.append(Client(
            ...
            ), partial(epoch, 'train', args=args, aug=False), args.device, args.init_trigger.clone().detach()
        ))
    ...
    # PLACEHOLDER FOR THE FUNCTION evaluate_syn - This is not used.
        ...
        for iteration in range(args.Iteration + 1):
            if iteration in eval_it_pool:
                ...
                model_eval_pool_accs = evaluate_syn_dataset(
                    ...
                    image_syn, label_syn, testloader, testloader_trigger, clients, clients_to_poison
                )
                print()
            ...
            for outer_iteration in range(args.outer_loop):
                ...
                if (outer_iteration + 1) % 10 == 0:
                    print(f'{get_time()}: Backdoor trigger updated using {args.trigger_update_strategy} strategy')
